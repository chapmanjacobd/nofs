_nofs() {
    ...
    do
        case "${cmd},${i}" in
            ...
            nofs__help,help)
                ...
                ;;
            nofs__help,info)
                cmd="nofs__help__info"
                ;;
            nofs__help,ls)
                cmd="nofs__help__ls"
                ;;
            nofs__help,manpage)
                cmd="nofs__help__manpage"
                ;;
            nofs__help,mkdir)
                cmd="nofs__help__mkdir"
                ;;
            nofs__help,mv)
                cmd="nofs__help__mv"
                ;;
            nofs__help,rm)
                ...
                ;;
            nofs__help,rmdir)
                cmd="nofs__help__rmdir"
                ;;
            nofs__help,stat)
                cmd="nofs__help__stat"
                ;;
            nofs__help,touch)
    ...
    case "${cmd}" in
        ...
        nofs__cat)
            ...
            ;;
        nofs__completions)
            opts="-c -v -h -V --config --paths --policy --minfreespace --verbose --json --help --version bash elvish fish powershell zsh"
            if [[ ${cur} == -* || ${COMP_CWORD} -eq 2 ]] ; then
                COMPREPLY=( $(compgen -W "${opts}" -- "${cur}") )
                return 0
            fi
        ...
        nofs__help__which)
            ...
            ;;
        nofs__info)
            opts="-c -v -h -V --config --paths --policy --minfreespace --verbose --json --help --version [CONTEXT]"
            if [[ ${cur} == -* || ${COMP_CWORD} -eq 2 ]] ; then
                COMPREPLY=( $(compgen -W "${opts}" -- "${cur}") )
                return 0
            fi
...
if [[ "${BASH_VERSINFO[0]}" -eq 4 && "${BASH_VERSINFO[1]}" -ge 4 || "${BASH_VERSINFO[0]}" -gt 4 ]]; then
    complete -F _nofs -o nosort -o bashdefault -o default nofs
else
    complete -F _nofs -o bashdefault -o default nofs
fi
